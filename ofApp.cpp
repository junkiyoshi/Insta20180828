...
void ofApp::update() {
	...
	for (int i = 0; i < this->circles.size(); i++) {
		...
		for (int j = i + 1; j < this->circles.size(); j++) {
			...
			if (distance < 80) {
				...
				this->circles[i]->addForce(this->circles[i]->getPosition() - this->circles[j]->getPosition(), 1.0);
				this->circles[j]->addForce(this->circles[j]->getPosition() - this->circles[i]->getPosition(), 1.0);
			}
		}
	}
	...
	for (int i = 0; i < this->circles.size(); i++) {
		...
		this->sizes[i] = ofMap(this->circles[i]->getVelocity().length(), 0, 8, 0, 1);
	}
